...
public class ModyfyReservationView {
	public ModyfyReservationView(String userid, int resNum) {
		...
		if(payment.equals("y") || payment.equals("Y") ) {
			while(credit+price_modify < price) {
				...
				if(credit+price_modify<price){
					System.out.println("취소시 잔액 : "+credit+"원");
				}
				flag = false;
			}
		}
		else {
			System.out.println("수정을 취소하셨습니다. 메인화면으로 돌아갑니다.");
			return;
		}
		if(credit+price_modify<price) {
			System.out.println("수정을 취소하셨습니다. 메인화면으로 돌아갑니다.");
		...
		rescon.deleteReservation(resNum);
